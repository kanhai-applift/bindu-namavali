<?php
require_once 'api-helper.php';

/* 3️⃣ Authorization check */
if (!in_array($_SESSION['role'], ['admin', 'superadmin'], true)) {
    http_response_code(403);
    respond('error', 'Unauthorized');
}

/* 4️⃣ Required field check */
if (empty($_POST['id']) || !is_string($_POST['id'])) {
    respond('error', 'Missing designation ID');
}

/* 5️⃣ Decode Hashid → numeric ID */
$hash = trim($_POST['id']);
$decoded = $hashids->decode($hash);

if (empty($decoded)) {
    respond('error', 'Invalid designation');
}

$designationId = (int) $decoded[0];
$orgId = (int) $_SESSION['user_id'];

if ($orgId <= 0 || $designationId <= 0) {
    respond('error', 'Invalid request');
}

try {

    /* 6️⃣ Perform scoped delete (numeric PK) */
    $stmt = $mysqli->prepare("
        DELETE FROM designations
        WHERE id = ? AND organization_id = ?
    ");

    if (!$stmt) {
        throw new Exception($mysqli->error);
    }

    $stmt->bind_param(
        'ii',
        $designationId,
        $orgId
    );

    if (!$stmt->execute()) {
        throw new Exception($stmt->error, $mysqli->errno);
    }

    /* 7️⃣ Detect tampering / non-existent record */
    if ($stmt->affected_rows === 0) {
        $stmt->close();
        respond('error', 'Delete failed or unauthorized');
    }

    $stmt->close();

    respond('success', 'Designation deleted');

} catch (Exception $e) {

    /* 8️⃣ Log real error internally */
    error_log($e->getMessage());

    /* FK constraint violation */
    if (in_array($e->getCode(), [1451, 1452], true)) {
        respond('error', 'Designation is in use and cannot be deleted');
    }

    respond('error', 'Server error');
}
