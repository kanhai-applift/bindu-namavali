...
function uploadPdfOptional(string $field, string $prefix, int $pdfSize): ?string
{
  ...
  if ($_FILES[$field]['error'] !== UPLOAD_ERR_OK) {
    respond('error', 'File upload error. Check file type. Max size '.$pdfSize.'Mb allowed');
  }
...
if (mb_strlen($remarks) > POST_REMARK_LENTH) {
  respond('error', 'Remarks too long');
}

$servicePdf = uploadPdfOptional('service_rules_pdf', 'service', POST_SERVICE_PDF_SIZE);
$layoutPdf  = uploadPdfOptional('layout_pdf', 'layout', POST_LAYOUT_PDF_SIZE);
$goshPdf    = uploadPdfOptional('goshwara_pdf', 'goshwara', POST_GOSHWARA_PDF_SIZE);
